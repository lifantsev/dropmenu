...
if [ -e "$in_fifo_path" ]; then
    lg I "removing existing fifo: $in_fifo_path"
    rm "$in_fifo_path" &> /dev/null || :
fi

if [ -e "$out_fifo_path" ]; then
    lg I "removing existing fifo: $out_fifo_path"
...
lg I "initializing fifo: $in_fifo_path"
mkdir -p "$(dirname "$in_fifo_path")"
mkfifo "$in_fifo_path"

lg I "initializing fifo: $out_fifo_path"
...
while true; do
    ...
    lg F "loop start: awaiting input from in_fifo[$in_fifo_path]"
